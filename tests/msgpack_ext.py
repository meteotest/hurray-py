...
import msgpack
import numpy as np
from hurraypy.msgpack_ext import encode, decode
...
class MsgPackTestCase(unittest.TestCase):
    def test_ndarray(self):
        ...
        packed_nparray = msgpack.packb(data_in, default=encode,
                                       use_bin_type=True)
        unpacked_nparray = msgpack.unpackb(packed_nparray, object_hook=decode,
                                           encoding='utf-8')
    ...
    def test_slice(self):
        ...
        packed_slice = msgpack.packb(slice_in, default=encode,
                                     use_bin_type=True)
        unpacked_slice = msgpack.unpackb(packed_slice, object_hook=decode,
                                         encoding='utf-8')
